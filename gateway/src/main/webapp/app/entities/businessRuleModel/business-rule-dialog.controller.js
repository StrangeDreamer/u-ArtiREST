...
angular.module('gatewayApp').controller('BusinessRuleModelDialogController',
    ['$scope', '$stateParams', '$uibModalInstance', 'entity', 'processModel', '$http',
        ...
        //allowedArtifacts实际上就是这个br能够用于什么artifact上，等同于brclass的value
        ...
        for(var i=0; i<$scope.artifactModelIds.length; i++) {
            $http.get('artifactmodel/api/artifact-models/' + $scope.artifactModelIds[i]).then(function (res) {
                $scope.artifactModels[i] = res.data;
                $scope.allowedArtifacts.push(res.data.name);
            })
        }
